...
export default function SearchBar() {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if ((e.metaKey || e.ctrlKey) && e.key === 'k' || (e.key === 'f' && !['INPUT', 'TEXTAREA'].includes((e.target as HTMLElement).tagName))) {
        e.preventDefault();
        setIsOpen(true);
      }
      // Esc to close
  ...
  const handleSearch = async (searchQuery: string) => {
    try {
      setLoading(true);
      const limit = 10;
      const response = await fetch(`/api/proxy/portal/search?keyword=${encodeURIComponent(searchQuery)}&limit=${limit}`);
  ...
  const hasResults = groupedResults.products.length > 0 || groupedResults.categories.length > 0 || groupedResults.pages.length > 0;
    ...
    <>
      ...
      <div className="relative z-[70] h-9 w-[200px]">
        ...
        >
          ...
          <div className={`flex items-center w-full ${isOpen ? 'p-3 border-b border-gray-100 h-14' : 'h-full px-3'}`}>
             <motion.svg 
             ...
             {isOpen ? (
                ...
                <div className="hidden sm:flex items-center">
                  <kbd className="h-5 px-1.5 pt-[1px] text-[10px] font-medium text-gray-400 bg-white border border-gray-200 rounded shadow-sm font-sans flex items-center justify-center">F</kbd>
                </div>
             )}
          </div>
                 ...
                 <div className="overflow-y-auto custom-scrollbar flex-1 p-2 border-t border-gray-100">
                    ...
                    {!loading && query && !hasResults && (
                      <div className="py-8 text-center text-gray-500">
                        <p>No results for "<span className="text-gray-900 font-medium">{query}</span>"</p>
                      </div>
                    )}
                    ...
                    {!query && !loading && (
                      <div className="space-y-4 pt-2">
                        ...
                        <div>
                          <h3 className="px-3 mb-1 text-xs font-semibold text-gray-500 uppercase tracking-wider">Suggested</h3>
                          <div>
                  ...
                  <div className="p-3 border-t border-gray-100 bg-gray-50 text-xs text-gray-500 flex items-center justify-between sticky bottom-0 bg-gray-50">
                    <div className="flex gap-4">
                      <span className="flex items-center gap-1">
                        <kbd className="font-sans px-1.5 py-0.5 bg-white border border-gray-200 rounded shadow-sm">↑</kbd>
                        <kbd className="font-sans px-1.5 py-0.5 bg-white border border-gray-200 rounded shadow-sm">↓</kbd>
                        <span>navigate</span>
                      </span>
                      <span className="flex items-center gap-1">
