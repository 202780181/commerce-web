 ...
 * 获取指定路径下的内容
 */
export function getItemsAtPath(productId: string, pathSegments: string[] = []): FileSystemItem[] {
 ...
 * 获取文件夹缩略图
 */
export function getFolderThumbnail(productId: string, pathSegments: string[]): string | null {
 ...
 * 获取面包屑导航
 */
export function getBreadcrumbs(productId: string, pathSegments: string[]): Breadcrumb[] {
 ...
 * 获取产品详情数据
 */
export function getProductDetail(productId: string, pathSegments: string[]): {
  name: string;
  folderName: string;
  ...
  // 分离 PNG 线条图和产品图片
  const lineDrawing = imageItems.find(item => item.fileName?.endsWith('.png'))?.url;
  const productImages = imageItems
    .filter(item => !item.fileName?.endsWith('.png'))
    .map(item => item.url || '');
  
  const descriptionUrl = textItems.find(item => item.fileName?.endsWith('.txt'))?.url;
